...
public class BasicLTISecurityFilter extends GenericFilterBean {
	...
	private BasicLtiContext buildBasicLtiContext(OAuthMessage requestMessage) throws IOException {
		...
		if (roles != null && roles.length() > 0) {
			context.getUser().setRoles(Arrays.asList(roles.split(",")));
		}
		...
		context.getOauth().setTimestamp(toLong(requestMessage.getParameter("oauth_timestamp"), 0));
	...
	private void handleBasicLtiRequest(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
		try {
			...
			if(consumerKey == null || consumerKey.length() == 0 || consumerSecret == null || consumerSecret.length() == 0) {
            	throw new OAuthProblemException("Empty consumer key or secret");
			}
